...
def cookie():
    ...
    resp.set_cookie('name_color', 'cornflowerblue', max_age=5)
    return resp
...
def ticket():
    ...
    if not request.args:
        print("DEBUG: Форма не отправлена - показываем пустую форму")
    ...
    print(f"DEBUG: Получены данные формы:")
    print(f"  ФИО: {fio}")
    print(f"  Полка: {berth}")
    print(f"  Возраст: {age}")
    ...
    print(f"  Дата: {date}")
    print(f"  Постельное белье: {linen}")
    print(f"  Багаж: {baggage}")
    print(f"  Страховка: {insurance}")
    print(f"  Все параметры: {dict(request.args)}")

    # Проверяем что все поля заполнены
    required_fields = [fio, berth, age, from_city, to_city, date]
    if not all(required_fields):
        missing_fields = []
        if not fio: missing_fields.append('fio')
        if not berth: missing_fields.append('berth')
        if not age: missing_fields.append('age')
        if not from_city: missing_fields.append('from_city')
        if not to_city: missing_fields.append('to_city')
        if not date: missing_fields.append('date')
        
        print(f"DEBUG: Ошибка - не заполнены поля: {missing_fields}")
        return "Все поля должны быть заполнены!", 400

    # Проверяем возраст
    try:
        age_int = int(age)
        print(f"DEBUG: Возраст преобразован в число: {age_int}")
        
        if not (1 <= age_int <= 120):
            print(f"DEBUG: Ошибка - возраст вне диапазона: {age_int}")
            return "Возраст должен быть от 1 до 120 лет!", 400
    except ValueError as e:
        print(f"DEBUG: Ошибка преобразования возраста '{age}': {e}")
        return "Возраст должен быть числом!", 400
    
    # Расчет цены
    is_child = age_int < 18
    price = 700 if is_child else 1000
    print(f"DEBUG: Базовая цена: {price} (ребенок: {is_child})")
    
    if berth in ['нижняя', 'нижняя боковая']:
        price += 100
        print(f"DEBUG: Добавлена надбавка за полку: +100")

    if linen:
        price += 75
        print(f"DEBUG: Добавлена стоимость белья: +75")
    if baggage:
        price += 250
        print(f"DEBUG: Добавлена стоимость багажа: +250")
    if insurance:
        price += 150
        print(f"DEBUG: Добавлена стоимость страховки: +150")

    print(f"DEBUG: Итоговая цена: {price}")

    # Отладочная информация перед рендерингом результата
    print(f"DEBUG: Передаем данные в шаблон:")
    print(f"  ФИО: {fio}")
    print(f"  Полка: {berth}")
    print(f"  Возраст: {age_int}")
    print(f"  Маршрут: {from_city} -> {to_city}")
    print(f"  Дата: {date}")
    print(f"  Доп. услуги: белье={linen}, багаж={baggage}, страховка={insurance}")
